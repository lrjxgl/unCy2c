<template>
	<view class="main-body">
		...
		<view v-else>
			...
			<view class="order-box">
				<view class="order-box-hd">
					...
					<view @click="gourl('../cy2c_order/my')" class="fr cor2 flex flex-center">全部订单
						<text class="iconfont icon-right"></text>
					</view>
				</view>
				<view class="order-box-status flex">
					<view @click="gourl('../cy2c_order/my?type=unpay')" class="order-box-item">
						<text class="iconfont icon-moneybag cl-u"></text> 待付款</view>
					 
					<view @click="gourl('../cy2c_order/my?type=unreceive')" class="order-box-item">
						<text class="iconfont icon-deliver cl-u"></text> 待收货</view>
					<view @click="gourl('../cy2c_order/my?type=unraty')" class="order-box-item">
		...
		<cy2c-footer tab="user"></cy2c-footer>
	</view>
</template>

<script>
	 
	import cy2cFooter from "../../components/cy2c-footer.vue";
	import cy2cReclist from "../../components/cy2c-reclist.vue";
	export default {
		components: {
			cy2cFooter,
			cy2cReclist
		...
		methods: {
			...
			getPage: function () {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/cy2c_user/index?ajax=1",
					unLogin:true,
