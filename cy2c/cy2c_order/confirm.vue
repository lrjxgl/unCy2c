		...
		<view v-if="pageLoad">
			...
			<view class="row-box mgb-5">
				<view class="flex flex-wrap flex-ai-center">
					<view>共{{pageData.total_num}}件商品， 总价：￥{{pageData.total_money}}</view>
				</view>
			</view>
			<!-- #ifdef H5 -->
			<view class="row-box mgb-5">
	...
	export default{
		...
		methods:{
			...
			changeAddr:function(e){				
				...
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/cy2c_order/confirm?ajax=1",
					data:{						
						cartid:that.cartid,
			...
			formSubmit:function(e){
				...
				var that=this;
				e.detail.value.backurl=that.app.appRoot+"#/pages/cy2c_order/success";
				that.app.post({
					url:that.app.apiHost+"/cy2c_order/order?ajax=1",
					data:e.detail.value,
					success:function(rs){
						if(rs.error){
							uni.showToast({
