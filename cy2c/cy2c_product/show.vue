<template>
	<view>
		<view v-if="pageLoad">
			...
			<cy2c-proraty :productid="pageData.data.id"></cy2c-proraty>
			...
			<cy2c-likelist :productid="pageData.data.id"></cy2c-likelist>
			<view class="h60"></view>
			...
			<view v-else class="flcart">
				...
				<navigator url="../cy2c_cart/index" class="flcart-f1">
...
<script>
	import cy2cProraty from "../../components/cy2c-proraty.vue";
	import tableFields from "../../components/tablefield/tablefield-show.vue";
	import cy2cLikelist from "../../components/cy2c-likelist.vue";
	export default {
		components: {
			cy2cProraty,
			tableFields,
		...
		methods: {
			...
			getPage: function(id) {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/cy2c_product/show?id=" + id,
					data: {
			...
			ks1: function(id) {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/cy2c_product_ks/sizeList?id=" + id,
					dataType: "json",
					success: function(res) {
						that.ksid1 = res.data.ksid;
						that.ksid = res.data.ksid;
			...
			ks2: function(id) {
				...
				that.ksid = id;
				that.app.get({
					url: that.app.apiHost + "/cy2c_product_ks/get?id=" + id,
					success: function(res) {
						that.ksproduct = res.data.product;
						that.price = res.data.ks.price;
						if(res.data.cart_amount==0){
			...
			addCart: function() {
				...
				var ksid = that.ksid;
				that.app.get({
					url: that.app.apiHost + '/cy2c_cart/add?ajax=1',
					data: {
					...
					success: function(res) {
						...
						if (that.goBuy) {
							uni.navigateTo({
								url: "../cy2c_order/confirm?cartid=" + res.data.cartid + "&ispin=" + that.gopin+"&orderid="+that.orderid
							})
						}
