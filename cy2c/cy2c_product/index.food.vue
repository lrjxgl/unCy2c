<template>
	<view>
		...
		<cy2c-footer tab="product"></cy2c-footer>
	</view>
</template>
...
<script>
	...
	var isFirst=true;
	var per_page=0;
	import cy2cFooter from "../../components/cy2cfooter.vue";
	export default{
		components:{
			cy2cFooter
		},
		...
		methods: {
			goProduct:function(id){
				uni.navigateTo({
					url:"../cy2c_product/show?id="+id
				})
			},
			getPage: function() {
				var that = this;
				that.app.get({
					url:that.app.apiHost+"/cy2c_product/index?ajax=1",
			...
			ksBox:function(id){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/cy2c_product_ks/index?productid="+id,
					success:function(res){
						if(res.error){
							uni.showToast({
								title:res.message
